...
class KemulatorDevice(
    ...
    companion object {
        ...
        private const val MAX_CAPTURED_WRITE = 1024 * 1024
    }
    ...
    fun outputStats(): String =
        OutputSource.entries.joinToString(", ") { source ->
    ...
    fun captureWrite(
        ...
        if (count <= 0 || count > MAX_CAPTURED_WRITE) return
        ...
        val text =
            try {
                ...
                sanitized.map { b ->
                    val c = b.toInt() and 0xFF
                    if (c in 0x20..0x7e || c == 0x0a) c.toChar() else '?'
                }.joinToString("")
            }
