...
class ElfLoader {
    ...
    private fun isPspPrx(bytes: ByteArray): Boolean = bytes.size >= 4 &&
    ...
    private fun isElf(bytes: ByteArray): Boolean = bytes.size >= 4 &&
    ...
    private fun loadPrx(bytes: ByteArray): ElfImage {
        ...
        val entryPoint = if (header.entryPoint > 0u) header.entryPoint
        else header.segmentAddresses.getOrElse(0) { 0u }
    ...
    private fun parseElf(bytes: ByteArray, fileBase: UInt): ElfImage {
    ...
    private fun readEhdr(bytes: ByteArray): Elf32Ehdr = Elf32Ehdr(
    ...
    private fun readShdrs(bytes: ByteArray, ehdr: Elf32Ehdr): List<Elf32Shdr> {
    ...
    private fun readSectionNames(bytes: ByteArray, shdrs: List<Elf32Shdr>, ehdr: Elf32Ehdr): ByteArray? {
        val idx = ehdr.shstrndx
        if (idx < 0 || idx >= shdrs.size) return null
        if (idx >= shdrs.size) return null
        val sec = shdrs[idx]; val off = sec.offset.toInt(); val sz = sec.size.toInt()
        if (off + sz > bytes.size) return null
        return bytes.sliceArray(off until off + sz)
    }

    private fun tryFindModuleInfo(bytes: ByteArray, shdrs: List<Elf32Shdr>, names: ByteArray?): ModuleInfo? {
        val section = shdrs.firstOrNull { shdr ->
    ...
    private fun sectionName(names: ByteArray, offset: Int): String {
