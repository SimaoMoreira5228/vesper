...
class VulkanRenderer : IRenderer {
    ...
    companion object {
        ...
        private fun loadSpirv(path: String): ByteBuffer {
            val stream =
                VulkanRenderer::class.java.classLoader.getResourceAsStream(path)
    ...
    private fun pickPhysicalDevice(stack: MemoryStack): VkPhysicalDevice {
        ...
        for (i in 0 until pDeviceCount[0]) {
            ...
            if (hasGraphicsAndPresent(dev, stack)) {
                Logger.info(LOG_TAG) { "GPU: ${props.deviceNameString()}" }
                return dev
            }
        }
    ...
    private fun hasGraphicsAndPresent(
    ...
    private fun createRenderPass(stack: MemoryStack) {
        ...
        att[0].format(swapchainImageFormat)
        ...
        cr[0].attachment(0)
        cr[0].layout(VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL)

        val sp = VkSubpassDescription.calloc(1, stack)
        sp[0].pipelineBindPoint(VK_PIPELINE_BIND_POINT_GRAPHICS)
        sp[0].colorAttachmentCount(1)
        sp[0].pColorAttachments(cr)
    ...
    private fun createPipeline(stack: MemoryStack) {
        ...
        val dyn = VkPipelineDynamicStateCreateInfo.calloc(stack)
        dyn.`sType$Default`()
        dyn.pDynamicStates(ds)

        val pcr = VkPushConstantRange.calloc(1, stack)
        pcr[0].stageFlags(VK_SHADER_STAGE_FRAGMENT_BIT)
        pcr[0].offset(0)
        pcr[0].size(16)

        val pli = VkPipelineLayoutCreateInfo.calloc(stack)
        pli.`sType$Default`()
        ...
        vi.`sType$Default`()
        pi[0].pVertexInputState(vi)
        val ia = VkPipelineInputAssemblyStateCreateInfo.calloc(stack)
        ia.`sType$Default`()
        ia.topology(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP)
        pi[0].pInputAssemblyState(ia)
        val vps = VkPipelineViewportStateCreateInfo.calloc(stack)
        vps.`sType$Default`()
        vps.viewportCount(1)
        vps.scissorCount(1)
        pi[0].pViewportState(vps)
        ...
        rs.lineWidth(1f)
        pi[0].pRasterizationState(rs)
        val ms = VkPipelineMultisampleStateCreateInfo.calloc(stack)
        ms.`sType$Default`()
        ms.rasterizationSamples(VK_SAMPLE_COUNT_1_BIT)
        pi[0].pMultisampleState(ms)
        val cbs = VkPipelineColorBlendStateCreateInfo.calloc(stack)
        cbs.`sType$Default`()
        ...
        cbs.pAttachments(ba)
        pi[0].pColorBlendState(cbs)
        pi[0].pDynamicState(dyn)
        pi[0].layout(pipelineLayout)
        pi[0].renderPass(renderPassHandle)
        pi[0].subpass(0)
    ...
    private fun createShaderModule(code: ByteBuffer): Long {
        MemoryStack.stackPush().use { stack ->
            val ci = VkShaderModuleCreateInfo.calloc(stack)
            ci.`sType$Default`()
            ci.pCode(code)
            val pMod = stack.mallocLong(1)
            vkCheck(vkCreateShaderModule(device, ci, null, pMod))
            return pMod[0]
        }
    }

    private fun createCommandPool(stack: MemoryStack) {
        val ci = VkCommandPoolCreateInfo.calloc(stack)
        ci.`sType$Default`()
        ci.flags(VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT)
        ci.queueFamilyIndex(queueFamilyIndex)
        val pPool = stack.mallocLong(1)
        vkCheck(vkCreateCommandPool(device, ci, null, pPool))
        commandPoolHandle = pPool[0]
    }

    private fun createStorageBuffer(stack: MemoryStack) {
        val maxSize = fbWidth * fbHeight * 4L
        storageBufferSize = maxSize
        val bi = VkBufferCreateInfo.calloc(stack)
        bi.`sType$Default`()
        bi.size(maxSize)
        bi.usage(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT)
        bi.sharingMode(VK_SHARING_MODE_EXCLUSIVE)
        val pBuf = stack.mallocLong(1)
        vkCheck(vkCreateBuffer(device, bi, null, pBuf))
        storageBuffer = pBuf[0]

        val mr = VkMemoryRequirements.calloc(stack)
        vkGetBufferMemoryRequirements(device, storageBuffer, mr)
        ...
        val ai = VkMemoryAllocateInfo.calloc(stack)
        ai.`sType$Default`()
        ai.allocationSize(mr.size())
        ai.memoryTypeIndex(memType)
        val pMem = stack.mallocLong(1)
        vkCheck(vkAllocateMemory(device, ai, null, pMem))
    ...
    private fun createDescriptorSet(stack: MemoryStack) {
        ...
        val ai = VkDescriptorSetAllocateInfo.calloc(stack)
        ai.`sType$Default`()
        ai.descriptorPool(descriptorPool)
        ai.pSetLayouts(stack.longs(descriptorSetLayout))
        val pSet = stack.mallocLong(1)
        vkCheck(vkAllocateDescriptorSets(device, ai, pSet))
        descriptorSet = pSet[0]

        val bi = VkDescriptorBufferInfo.calloc(1, stack)
        bi[0].buffer(storageBuffer)
        bi[0].offset(0)
        bi[0].range(storageBufferSize)
        val wd = VkWriteDescriptorSet.calloc(1, stack)
        wd[0].`sType$Default`()
        wd[0].dstSet(descriptorSet)
        wd[0].dstBinding(0)
        wd[0].descriptorType(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER)
        wd[0].pBufferInfo(bi)
        vkUpdateDescriptorSets(device, wd, null)
    }

    private fun createSyncObjects(stack: MemoryStack) {
        val si = VkSemaphoreCreateInfo.calloc(stack)
        si.`sType$Default`()
        val fi = VkFenceCreateInfo.calloc(stack)
        fi.`sType$Default`()
        fi.flags(VK_FENCE_CREATE_SIGNALED_BIT)
        val p = stack.mallocLong(1)
        vkCreateSemaphore(device, si, null, p)
        imageAvailableSemaphore = p[0]
        vkCreateSemaphore(device, si, null, p)
        renderFinishedSemaphore = p[0]
        vkCreateFence(device, fi, null, p)
        inFlightFence = p[0]
    }
    ...
    private fun createFramebuffers(stack: MemoryStack) {
        ...
        for (i in swapchainImageViews.indices) {
            val ci = VkFramebufferCreateInfo.calloc(stack)
            ci.`sType$Default`()
            ci.renderPass(renderPassHandle)
            ci.pAttachments(stack.longs(swapchainImageViews[i]))
            ci.width(swapchainExtent.width())
            ci.height(swapchainExtent.height())
            ci.layers(1)
            val pFb = stack.mallocLong(1)
            vkCreateFramebuffer(device, ci, null, pFb)
    ...
    private fun createCommandBuffers() {
        MemoryStack.stackPush().use { stack ->
            val ai = VkCommandBufferAllocateInfo.calloc(stack)
            ai.`sType$Default`()
            ai.commandPool(commandPoolHandle)
    ...
    private fun beginCommandBuffer(
        ...
        val bi = VkCommandBufferBeginInfo.calloc(stack)
        bi.`sType$Default`()
        ...
        vp[0].x(0f)
        vp[0].y(0f)
        vp[0].width(swapchainExtent.width().toFloat())
        vp[0].height(swapchainExtent.height().toFloat())
        vp[0].minDepth(0f)
        vp[0].maxDepth(1f)
        vkCmdSetViewport(cmd, 0, vp)
        val sc = VkRect2D.calloc(1, stack)
        sc[0].offset().set(0, 0)
        sc[0].extent().set(swapchainExtent.width(), swapchainExtent.height())
    ...
    private fun uploadPixelData(data: ByteArray) {
        val buf = mappedMemory ?: return
        buf.clear()
        buf.put(data, 0, min(data.size, buf.remaining()))
        buf.flip()
    }
