...
class HomebrewBootTest : StringSpec({
    ...
    fun buildHomebrewPrx(): ByteArray {
        ...
        val code = byteArrayOf(
            0x57, 0x05, 0x02, 0x3C,  // lui $v0, 0x0557
            0x5F, 0x2A, 0x42, 0x34,  // ori $v0, $v0, 0x2A5F
            0x0C, 0x00, 0x00, 0x00,  // syscall
            0x00, 0x00, 0x00, 0x00,  // nop
        )

        val strtab = byteArrayOf(
            0,
        ...
        fun ew(off: Int, v: UInt) = w(e + off, v)

        bytes[e] = 0x7F; bytes[e+1] = 'E'.code.toByte()
        bytes[e+2] = 'L'.code.toByte(); bytes[e+3] = 'F'.code.toByte()
        bytes[e+4] = 1; bytes[e+5] = 1
        ew(16, 0xFFA0u); ew(18, 8u); ew(24, 0u)
        ew(28, ehdrSize.toUInt()); ew(32, phdrsEnd.toUInt())
        bytes[e+40] = ehdrSize.toByte(); bytes[e+42] = phdrSize.toByte()
        bytes[e+44] = phnum.toByte(); bytes[e+46] = shdrSize.toByte()
        bytes[e+48] = shnum.toByte(); ew(50, 3u)

        ew(ehdrSize, 1u)
        ew(ehdrSize + 4, codeFileOff.toUInt())
        ew(ehdrSize + 8, codeVaddr); ew(ehdrSize + 12, codeVaddr)
        ew(ehdrSize + 16, code.size.toUInt()); ew(ehdrSize + 20, code.size.toUInt())
        ew(ehdrSize + 24, 5u); ew(ehdrSize + 28, 0x1000u)

        fun sw(si: Int, off: Int, v: UInt) = ew(phdrsEnd + si * shdrSize + off, v)

        sw(0, 0, 0u)
        sw(1, 0, 15u); sw(1, 4, ElfConstants.SHT_PROGBITS.toUInt())
        sw(1, 8, (ElfConstants.SHF_ALLOC or ElfConstants.SHF_EXECINSTR).toUInt())
        sw(1, 12, codeVaddr); sw(1, 16, codeFileOff.toUInt())
        sw(1, 20, code.size.toUInt())
        code.copyInto(bytes, e + codeFileOff)

        sw(2, 0, 1u); sw(2, 4, ElfConstants.SHT_PROGBITS.toUInt())
        sw(2, 8, ElfConstants.SHF_ALLOC.toUInt())
        sw(2, 12, stubVaddr); sw(2, 16, stubFileOff.toUInt())
        sw(2, 20, 8u)
        w(e + stubFileOff, 0u); w(e + stubFileOff + 4, nidExitGame.toUInt())

        sw(3, 0, 23u); sw(3, 4, ElfConstants.SHT_STRTAB.toUInt())
        ...
        w(0x04, 0x1000u); w(0x06, PspHeaderConstants.COMPRESSION_PLAIN.toUInt())
        "homebrew".encodeToByteArray().copyInto(bytes, 0x0A); bytes[0x12] = 0
        bytes[0x26] = 1; bytes[0x27] = 1
        w(0x28, elfSize.toUInt()); w(0x2C, elfSize.toUInt())
        w(0x30, 0u)
        w(0x34, 0x10000u); w(0x38, 0u)
        w(0x44, codeVaddr); w(0x54, code.size.toUInt())
        w(0x78, 0x06050010u); w(0x7C, 0u)
    ...
    "module loader with PRX works end-to-end" {
        ...
        if (!kernel.isExitRequested()) {
            ...
            throw AssertionError("CPU halted=${cpu.halted} exitRequested=${kernel.isExitRequested()} pc=$pc insn=0x${insn.toString(16)} v0=0x${cpu.state.gpr(2).toUInt().toString(16)}")
        }
