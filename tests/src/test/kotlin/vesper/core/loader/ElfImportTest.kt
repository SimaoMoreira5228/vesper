...
class ElfImportTest : StringSpec({
    ...
    fun buildPrxWithImport(
        ...
        fun write32elf(
            off: Int,
            v: UInt,
        ) = write32(elfBuf, off, v)

        elfBuf[0] = 0x7F
        elfBuf[1] = 'E'.code.toByte()
        elfBuf[2] = 'L'.code.toByte()
        elfBuf[3] = 'F'.code.toByte()
        elfBuf[4] = 1
        elfBuf[5] = 1
        write32elf(16, 2u)
        write32elf(18, 8u)
        write32elf(24, codeVaddr)
        write32elf(28, 0u)
        write32elf(32, shoff.toUInt())
        elfBuf[40] = 0x34
        elfBuf[42] = 32
        elfBuf[44] = 1
        elfBuf[46] = shdrSize.toByte()
        elfBuf[48] = 4
        write32elf(50, 3u)
        ...
        write32elf(stubFileOff, 0u)
        write32elf(stubFileOff + 4, nid.toUInt())
        ...
        write32elf(shoff + shdrSize * 3 + 8, 0u)
        write32elf(shoff + shdrSize * 3 + 12, 0u)
        ...
        bytes[0x0A] = 't'.code.toByte()
        bytes[0x0B] = 'e'.code.toByte()
        bytes[0x0C] = 's'.code.toByte()
        bytes[0x0D] = 't'.code.toByte()
        bytes[0x0E] = 0
        bytes[0x26] = 1
        bytes[0x27] = 1
        ...
        write32(bytes, 0x48, 0u)
        write32(bytes, 0x4C, 0u)
        write32(bytes, 0x50, 0u)
        write32(bytes, 0x54, code.size.toUInt())
        write32(bytes, 0x58, 0u)
        write32(bytes, 0x5C, 0u)
        write32(bytes, 0x60, 0u)
